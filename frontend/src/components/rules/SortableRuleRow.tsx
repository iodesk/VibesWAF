...
import { Play, Pause, SquarePen, Shield, Trash2, GripVertical, Loader2 } from 'lucide-react';
...
export function SortableRuleRow({ rule, onDelete, onToggle, onEdit, isReordering, triggerCount }: SortableRuleRowProps) {
  ...
  return (
    ...
    >
      ...
      <td className="py-4 px-6 w-16">
        <Badge variant="outline" className="font-mono text-[10px] py-0 px-2 h-5 badge-tint">
          #{rule.priority}
        </Badge>
      </td>
      <td className="py-4 px-6" style={{ minWidth: '192px', maxWidth: '192px' }}>
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <span
            ...
            onClick={() => onEdit ? onEdit(rule) : navigate(`/rules-engine/edit/${rule.id}`)}
            title={rule.name}
          >
            {rule.name}
          </span>
      ...
      <td className="py-4 px-6 w-24">
        <span className="font-mono text-[12px] py-0 px-2 h-5 text-foreground">
          {triggerCount || 0}
        </span>
      </td>
      ...
      <td className="py-4 px-6 w-32">
        {rule.enabled ? (
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full status-active rounded-full" />
      ...
      <td className="py-4 px-6 text-right w-32">
        <div className="flex items-center justify-end gap-1">
          <Button
            ...
            title={rule.enabled ? 'Pause' : 'Start'}
