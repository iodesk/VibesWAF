...
export const CertificateDetailsDialog = ({
  ...
  const handleValidate = async () => {
    try {
      ...
      addToast('Certificate validated successfully', 'success');
      onRefresh();
    } catch (err) {
  ...
  const handleRenew = async () => {
    try {
      ...
      addToast('Renewal initiated successfully', 'success');
      onRefresh();
    } catch (err) {
      addToast(err instanceof Error ? err.message : 'Renewal failed', 'error');
    } finally {
      setRenewing(false);
    }
  };
  ...
  const isBusy = loading || validating || renewing;
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm">
                  {certificate.status === 'pending' ? '—' : formatDate(certificate.expires_at)}
                </p>
              </div>
